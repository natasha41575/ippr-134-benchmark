...
wait_for_resize_to_finish() {
    local local_expected_mem=$1 # First argument is expected memory
    local local_expected_cpu=$2 # Second argument is expected CPU
    echo "Waiting for container '$CONTAINER_NAME' in pod '$POD_NAME' to have mem request of '$local_expected_mem' and cpu request of '$local_expected_cpu'..."
    ...
    while true; do
        ...
        if [ "$actual_mem" = "$local_expected_mem" ] && [ "$actual_cpu" = "$local_expected_cpu" ]; then
            echo "Watching pod '$POD_NAME' for container '$CONTAINER_NAME' to have mem request of '$local_expected_mem' and cpu request of '$local_expected_cpu'."
            echo "---------------------------------------------------------"
...
do
    ...
    echo "increasing mem 2Gi -> 3Gi and cpu 1000m -> 1150m"
    kubectl patch pod "$POD_NAME" \
    -p '{"spec":{"containers":[{"name":"'"$CONTAINER_NAME"'","resources":{"requests":{"memory":"3Gi", "cpu":"1150m"},"limits":{"memory":"3Gi", "cpu":"1150m"}}}]}}' --subresource=resize

    echo "waiting for resize to succeed"
    start_ns=$(date +%s%N)
    wait_for_resize_to_finish "3Gi" "1150m"
    end_ns=$(date +%s%N)
    duration_ns=$((end_ns - start_ns))
    duration_ms=$((duration_ns / 1000000))
    echo "upscale resize succeeded after $duration_ms ms"

    echo "decreasing mem 3Gi -> 2Gi and cpu 1150m -> 1000m"
    kubectl patch pod "$POD_NAME" \
    -p '{"spec":{"containers":[{"name":"'"$CONTAINER_NAME"'","resources":{"requests":{"memory":"2Gi", "cpu":"1000m"},"limits":{"memory":"2Gi", "cpu":"1000m"}}}]}}' --subresource=resize

    echo "waiting for resize to succeed"
    start_ns=$(date +%s%N)
    wait_for_resize_to_finish "2Gi" "1"  
    end_ns=$(date +%s%N)
    duration_ns=$((end_ns - start_ns))
    duration_ms=$((duration_ns / 1000000))
